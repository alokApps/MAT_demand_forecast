...
# --- Main App ---
st.title("📈 SKU-Level Demand Forecasting & Production Planning")
st.markdown("A dashboard to present the model methodology, run the forecast, and analyze the results.")

# --- Create Tabs ---
tab_overview, tab_run, tab_validation, tab_forecast, tab_plan = st.tabs([
    "Project Overview", 
    ...
    if st.button("Start Forecast Pipeline", type="primary"):
        with st.spinner("Running... This will take a while. Please wait."):
            try:
                ...
                st.balloons()
                st.info("You can now view all generated outputs in the new tabs.")
...
# --- Tab 3: Validation & Scorecard ---
with tab_validation:
    st.header("Analyze Validation Results")
    st.markdown("This tab shows the model bake-off results, comparing each model's performance.")
    ...
    if not os.path.exists(VALIDATION_RESULTS_CSV):
        ...
        if df_validation is not None:
            ...
            col1, col2 = st.columns([1, 2]) # 1 part for selector, 2 parts for plot

            with col1:
                sku_list = df_validation['Material'].unique()
                selected_sku = st.selectbox("Select an SKU to view its plot:", sku_list)
...
with tab_forecast:
    st.header("Final 6-Month Forecast (Units)")
    ...
    if df_forecast is not None:
        ...
        st.info("No forecast file found. Please run the forecast pipeline in the 'Run Forecast Pipeline' tab first.")

# --- Tab 5: Production Plan ---
with tab_plan:
    st.header("Actionable Production Plan")
    ...
    if df_plan is not None:
        ...
        st.info("No production plan file found. Please run the forecast pipeline in the 'Run Forecast Pipeline' tab first.")